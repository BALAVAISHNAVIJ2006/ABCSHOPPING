...
// Handle subcategory filter for menswear
$subcategory = isset($_GET['subcategory']) ? $_GET['subcategory'] : '';

$result = null;
if (!empty($subcategory)) {
    $sql = "SELECT id, name, category, price, description, image FROM dresses WHERE category = 'Menswear'";
    
    switch($subcategory) {
        case 'shirts':
            $sql .= " AND (name LIKE '%shirt%' OR name LIKE '%t-shirt%' OR name LIKE '%polo%')";
            break;
        case 'pants':
            $sql .= " AND (name LIKE '%pant%' OR name LIKE '%jean%' OR name LIKE '%trouser%' OR name LIKE '%chino%')";
            break;
        case 'jackets':
            $sql .= " AND (name LIKE '%jacket%' OR name LIKE '%blazer%' OR name LIKE '%coat%' OR name LIKE '%hoodie%')";
            break;
    }
...
<head>
    ...
    <title>Premium Menswear Collection - ABC Shopping</title>
...
<body>
            ...
            <a href="#" class="logo">ABC SHOPPING</a>
                <?php if ($isLoggedIn && $userInfo): ?>
                    <div class="user-menu">
                        ...
                        <a href="logout.php" style="color: #b21f1f;"><i class="fas fa-sign-out-alt"></i> Logout</a>
                    </div>
                <?php else: ?>
    ...
    <div class="container">
        <div class="header">
            ...
            <p>Explore our curated selection of premium menswear</p>
        </div>
        
        <div class="category-filter">
            <a href="?subcategory=shirts" class="filter-btn <?php echo $subcategory === 'shirts' ? 'active' : ''; ?>">
                <i class="fas fa-shirt"></i> Shirts
            </a>
            <a href="?subcategory=pants" class="filter-btn <?php echo $subcategory === 'pants' ? 'active' : ''; ?>">
                <i class="fas fa-tshirt"></i> Pants
            </a>
            <a href="?subcategory=jackets" class="filter-btn <?php echo $subcategory === 'jackets' ? 'active' : ''; ?>">
                <i class="fas fa-jacket"></i> Jackets
            </a>
        </div>
        ...
        <div class="products-grid">
            ...
            if (empty($subcategory)) {
                ...
                echo '<div class="welcome-message-icon"><i class="fas fa-shirt"></i></div>';
                echo '<h3>Welcome to Our Menswear Collection</h3>';
                echo '<p>Select a category above to explore our premium products</p>';
                echo '</div>';
            } elseif ($result && $result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    echo '<div class="product-card">';
                    echo '<div class="product-image" onclick="redirectToItemPage(\'' . htmlspecialchars($row['id']) . '\')">';
                    if (!empty($row['image'])) {
                        echo '<img src="' . htmlspecialchars($row['image']) . '" alt="' . htmlspecialchars($row['name']) . '">';
                    } else {
                        echo '<div class="product-placeholder"><i class="fas fa-shirt"></i></div>';
                    }
                    echo '</div>';
                    echo '<div class="product-info">';
                    echo '<div class="product-category">Menswear</div>';
    ...
    <script>
        function redirectToItemPage(productId) {
            window.location.href = 'items.php?id=' + encodeURIComponent(productId);
        }
